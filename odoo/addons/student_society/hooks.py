...
class SocietyImporter:
    ...
    def _read_csv(self, filename):
        ...
        if not path.exists():
            _logger.warning("Student Society import skipped missing file: %s", path)
            return []
    ...
    def _import_members(self):
        ...
        _logger.info("Student Society imported %s member rows", imported)
    ...
    def _import_projects(self):
        ...
        for filename in PROJECT_CSVS:
            for row in self._read_csv(filename):
                ...
                imported += 1
        _logger.info("Student Society imported %s project rows", imported)
    ...
    def _link_project_participants(self):
        ...
        for partner_id, project_names in self.member_project_mentions.items():
            for project_name in project_names:
                ...
                if not candidates:
                    continue
                project = sorted(candidates, key=lambda item: (item.date_start or fields.Date.from_string("1900-01-01"), item.id), reverse=True)[0]
                ...
                if not existing:
                    ...
                    linked += 1
        _logger.info("Student Society linked %s project participants from member project mentions", linked)

    def _index_project(self, project):
        keys = {
            normalize(project.name),
            normalize(project.client_id.name),
        }
        for key in keys:
            if key:
    ...
    def _group_for_role(self, position, department):
        ...
        if position_key in {"president", "vicepresident", "vicepresident"}:
...
def parse_date(value):
    ...
    for date_format in ("%Y-%m-%d", "%m/%d/%Y", "%d/%m/%Y"):
        try:
            ...
            continue
    _logger.warning("Could not parse date value: %s", value)
    return False
